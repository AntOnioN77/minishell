...
static char	*form_path(char *cmd, char **enpath, enum e_errors *err, int i)
{
	char	*pathcmd;
	char	*slash;
...
static char	*match_pathcmd(char *cmd, char **enpath, enum e_errors *err)
{
	char	*pathcmd;
	int		i;
...
static int	is_absolutepath(char *cmd, enum e_errors *err)
{
	struct stat	tipe;
